...
export default function AdminPage() {
  ...
  const loadData = useCallback(async () => {
    const { data: t } = await supabase.from('report_templates').select('*').order('created_at', { ascending: false });
  ...
  const getSubmissionForTeam = (templateId: string, teamNum: number) => {
    return submissions.find((s: any) => s.report_template_id === templateId && s.team_number === teamNum);
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(135deg, #0a1628 0%, #162d54 50%, #0f2240 100%)' }}>
      ...
      <header className="border-b border-slate-700/30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            ...
            <a href="/" target="_blank" className="text-xs text-blue-400 hover:text-blue-300 transition">
              View Dashboard ↗
            </a>
            <button
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section>
          <h2 className="text-lg font-bold text-white mb-4">📋 Report Templates & Submissions</h2>

          {templates.length === 0 ? (
            <div className="glass-card p-12 text-center">
              <span className="text-4xl block mb-3">📝</span>
              <p className="text-slate-400">No report templates yet. Add your first report above.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {templates.map((tpl: any) => (
                <div key={tpl.id} className="glass-card p-4 sm:p-5">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                    ...
                    <button
                      onClick={() => handleDeleteReport(tpl.id)}
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {[1, 2, 3, 4].map((teamNum) => {
                      ...
                      return (
                        <button
                          key={teamNum}
                          onClick={() => handleToggleSubmission(tpl.id, teamNum)}
                        ...
                        >
                          ...
                          {submitted && <span className="ml-auto text-xs">✓</span>}
                        </button>
                      );
                    })}
                  </div>
